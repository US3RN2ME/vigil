...
std::filesystem::path tempAlertPath() {
  return std::filesystem::temp_directory_path() /
         "vigil_alert_sink_test.ndjson";
}

std::filesystem::path tempConfigPath() {
  return std::filesystem::temp_directory_path() /
         "vigil_alert_sink_config_test.json";
}
...
suite<"[AlertSink]"> _ = [] {
  "[JsonFileSinkWritesNewlineDelimitedJson]"_test = [] {
...

    {
      ...
      file << R"({"alertSinks": [{"type": "json_file", "path": ")"
           << alertPath.generic_string() << R"("}], "rules": {}})";
    }

    {
      auto dispatcher = vigil::createAlertDispatcher(
          vigil::Config::loadFromFile(configPath.string()));

      vigil::ProcessInfo info{.pid = 42,
                              .name = "cmd.exe",
                              .exePath = R"(C:\Windows\System32\cmd.exe)"};
      const vigil::Alert alert{
          .rule = "test_rule", .severity = "high", .info = info};
