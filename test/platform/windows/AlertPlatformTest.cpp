...
suite<"[AlertPlatform]"> _ = [] {
  "[SerializesWindowsProcessMetadata]"_test = [] {
    ...
    const auto platform =
        nlohmann::json::parse(alert.toJson()).at("process").at("platform");

    expect(
        eq(platform.at("integrity").get<std::string>(), std::string{"high"}));
    expect(eq(platform.at("job_object_name").get<std::string>(),
              std::string{"test-job"}));
  };
};
} // namespace
