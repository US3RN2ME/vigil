...
suite<"[Alert]"> _ = [] {
  "[SerializesProcessSnapshotAndAttributes]"_test = [] {
    ...
    expect(
        eq(json.at("rule").get<std::string>(), std::string{"suspicious_test"}));
    ...
    expect(eq(json.at("process").at("command_line_original_length"),
              info.cmdline.size()));
    expect(eq(json.at("attributes").at("indicator").get<std::string>(),
              std::string{"quoted \"value\""}));
    expect(eq(json.at("attributes").at("empty").get<std::string>(),
              std::string{}));
  };
  ...
  "[TruncatesLongCommandLine]"_test = [] {
    ...
    expect(eq(process.at("command_line").get<std::string>().size(),
              std::size_t{256}));
