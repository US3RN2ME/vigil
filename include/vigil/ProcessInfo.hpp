...
namespace vigil {
   ...
   struct ProcessInfo {
      ...
      /**
       * @brief Short process name — limited by the kernel, not a reliable
       * identifier. Linux : task_struct.comm, max 15 chars + NUL. Windows:
       * EPROCESS.ImageFileName, max 15 chars + NUL. Use @ref exePath for a
       ...
       * Linux : /proc/&lt;pid&gt;/cmdline, NUL-separated, converted to
       * space-separated. Windows: PEB.ProcessParameters.CommandLine
      ...
      /**
       * @brief The executable path resolved for the process no longer exists on
       * disk. Linux: true for deleted executables. Windows: true when the image
       * path cannot be found during the snapshot. This is a high-signal image
       * backing anomaly, but not proof of maliciousness by itself.
       */
      bool imageMissingFromDisk = false;

      /**
       * @brief At least one anonymous (file-backed == false) memory region is
       * mapped with both WRITE and EXECUTE permissions simultaneously. Strong
       * indicator of shellcode injection or process hollowing. Linux : detected by
       * parsing /proc/&lt;pid&gt;/maps for rwxp entries with no path. Windows:
       * detected by VirtualQueryEx scanning for PAGE_EXECUTE_READWRITE.
       */
      bool hasAnonRwx = false;

      /**
       * @brief Process creation timestamp in nanoseconds since boot (monotonic
       * clock). Used for event correlation and process-tree ordering. Linux :
       * starttime field from /proc/&lt;pid&gt;/stat * (1e9 / CONFIG_HZ). Windows:
       * EPROCESS.CreateTime converted to nanoseconds.
       */
      ...
      /**
       * @brief Process called connect(2) to a non-loopback AF_INET/AF_INET6
       * address.
       */
