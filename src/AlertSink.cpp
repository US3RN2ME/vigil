...
public:
  explicit JsonFileSink(const std::string &path) : out_{path, std::ios::app} {
    if (!out_)
...
std::string jsonString(const nlohmann::json &json, std::string_view pointer) {
  const auto &value =
      json.at(nlohmann::json::json_pointer{std::string{pointer}});
...
std::string formatCef(const vigil::Alert &alert) {
  ...
  return std::format(
      "CEF:0|vigil|vigil|1|{}|{}|{}|dproc={} sproc={} cs1Label=ExecutablePath "
      "cs1={} cs2Label=CommandLine cs2={} "
      "cs3Label=ParentPid cs3={}",
      escapeCef(std::string{alert.rule}), escapeCef(std::string{alert.rule}),
      escapeCef(std::string{alert.severity}), escapeCef(name), escapeCef(pid),
      escapeCef(path), escapeCef(cmdline), escapeCef(ppid));
}
...
std::string formatLeef(const vigil::Alert &alert) {
  ...
  return std::format("LEEF:2.0|vigil|vigil|1|{}|sev={}\tusrName={}\tprocName={}"
                     ...
                     escapeLeef(std::string{alert.rule}),
                     escapeLeef(std::string{alert.severity}), escapeLeef(name),
                     escapeLeef(name), escapeLeef(pid), escapeLeef(ppid),
                     escapeLeef(path), escapeLeef(cmdline));
}

class SyslogSink final : public vigil::AlertSink {
public:
  explicit SyslogSink(vigil::SyslogAlertFormat format) : format_{format} {}

  void emit(const vigil::Alert &alert) override {
    const auto line = format_ == vigil::SyslogAlertFormat::Cef
                          ? formatCef(alert)
                          : formatLeef(alert);
...
std::unique_ptr<vigil::AlertSink>
createSink(const vigil::AlertSinkConfig &config) {
  switch (config.type) {
