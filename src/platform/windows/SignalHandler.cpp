...
void SignalHandler::wait() {
  ...
  stopRequestedCv.wait(lock, [this] { return stopRequested_; });
}

void SignalHandler::install() {
  SignalHandler *expected = nullptr;
  if (!activeHandler.compare_exchange_strong(expected, this,
...
void SignalHandler::uninstall() {
  ...
  if (!activeHandler.compare_exchange_strong(expected, nullptr,
                                             std::memory_order_acq_rel)) {
    return;
  }
...
void SignalHandler::notifyStopRequested() { stopRequestedCv.notify_all(); }
} // namespace vigil
