...
std::optional<Event> Event::create(bool manualReset,
...
WaitResult Event::wait(uint32_t milliseconds) const noexcept {
  switch (WaitForSingleObject(static_cast<HANDLE>(handle_.native()),
                              milliseconds)) {
  case WAIT_OBJECT_0:
...
Event::Event(Handle handle) noexcept : handle_{std::move(handle)} {}
} // namespace vigil::platform
