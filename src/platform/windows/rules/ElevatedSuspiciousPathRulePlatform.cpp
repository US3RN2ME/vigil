...
namespace {
std::string
integrityToString(vigil::ProcessInfo::PlatformInfo::Integrity integrity) {
...
bool isElevatedProcess(const ProcessInfo &info) {
  ...
  return info.platform.integrity == Integrity::High ||
         info.platform.integrity == Integrity::System;
}

void appendElevationAttributes(Alert &alert, const ProcessInfo &info) {
  alert.attributes.emplace_back("integrity",
                                integrityToString(info.platform.integrity));
}
} // namespace vigil::rules::detail
