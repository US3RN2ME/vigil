...
bool isProtectedName(const vigil::rules::RuleConfig &cfg,
                     std::string_view name) {
  return std::ranges::any_of(cfg.protectedProcessNames,
                             [name](const auto &protectedName) {
...
SystemProcessImpersonationRule::SystemProcessImpersonationRule(
...
std::optional<Alert>
SystemProcessImpersonationRule::check(const ProcessInfo &info) {
  const auto processName = lower(info.name);
  if (!isProtectedName(cfg_, processName))
    return {};

  const auto path = lower(info.exePath);
  const bool inSystemRoot = path.starts_with("c:\\windows\\system32\\") ||
...
Alert SystemProcessImpersonationRule::makeAlert(const ProcessInfo &info) const {
  ...
  alert.attributes = {
      {"expected_path", R"(C:\Windows\System32\ or C:\Windows\SysWOW64\)"}};
  return alert;
}
} // namespace vigil::platform::rules
