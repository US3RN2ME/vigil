...
std::string matchedTechnique(const vigil::ProcessInfo &info) {
  ...
  if (name == "rundll32.exe" &&
      containsAny(cmdline, {"javascript:", "url.dll,fileprotocolhandler",
                            "http://", "https://"}))
    return "rundll32_remote_or_script_handler";
  if (name == "regsvr32.exe" &&
      containsAny(cmdline, {"/i:http", "/i:https", "scrobj.dll"}))
    return "regsvr32_remote_scriptlet";
  if (name == "mshta.exe" &&
      containsAny(cmdline, {"http://", "https://", "vbscript:", "javascript:"}))
    return "mshta_remote_or_inline_script";
  if (name == "certutil.exe" &&
      containsAny(cmdline, {" -urlcache ", " -decode ", " -decodehex "}))
    return "certutil_download_or_decode";
  if (name == "bitsadmin.exe" &&
      containsAny(cmdline, {" /transfer ", " /create ", "http://", "https://"}))
    return "bitsadmin_transfer";
  if ((name == "wscript.exe" || name == "cscript.exe") &&
      containsAny(cmdline, {"http://", "https://", "\\appdata\\local\\temp\\",
                            "\\windows\\temp\\"}))
...
std::string_view LolbinExecutionRule::name() const noexcept { return kName; }
