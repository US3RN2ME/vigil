...
namespace vigil::platform::rules {
   ...
   std::optional<Alert> SuspiciousPowerShellRule::check(const ProcessInfo& info) {
      ...
      const bool suspicious = contains(cmdline, "-encodedcommand") || contains(cmdline, "frombase64string") ||
                              contains(cmdline, "downloadstring") || contains(cmdline, "invoke-expression") ||
                              contains(cmdline, " iex ") || contains(cmdline, "-nop") || contains(cmdline, "-w hidden") ||
