...
struct TcpConnectionHash {
  std::size_t operator()(const TcpConnection &connection) const noexcept {
    ...
    hash ^= std::hash<uint16_t>{}(connection.remotePort) + 0x9e3779b9 +
            (hash << 6) + (hash >> 2);
    hash ^= std::hash<std::string>{}(connection.remoteAddr) + 0x9e3779b9 +
            (hash << 6) + (hash >> 2);
    return hash;
  }
};
...
public:
  static void
  forEach(const std::function<void(const TcpConnection &)> &callback);
};
