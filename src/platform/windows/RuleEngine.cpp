...
namespace vigil {
   std::unique_ptr<RuleEngine> createRuleEngine(Config cfg) {
      ...
      engine->addRule(std::make_unique<SystemProcessImpersonationRule>(cfg.rule(SystemProcessImpersonationRule::kName)));
      engine->addRule(std::make_unique<LolbinExecutionRule>(cfg.rule(LolbinExecutionRule::kName)));
      engine->addRule(std::make_unique<SuspiciousPowerShellRule>(cfg.rule(SuspiciousPowerShellRule::kName)));
      return engine;
   }
} // namespace vigil
