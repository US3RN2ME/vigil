...
std::optional<EtwSession>
EtwSession::start(std::wstring sessionName,
                  std::array<uint8_t, 16> providerGuid, uint64_t keyword) {
  EtwSession session;
  ...
  // Stop any leftover session with the same name before starting fresh.
  {
    ...
    ControlTraceW(0, session.name_.c_str(), &staleProps.props,
                  EVENT_TRACE_CONTROL_STOP);
  }

  TRACEHANDLE sessionHandle{};
  auto props = makeProperties(session.name_);
  const ULONG startErr =
      StartTraceW(&sessionHandle, session.name_.c_str(), &props.props);
  ...
  const ULONG enableErr =
      EnableTraceEx2(sessionHandle, &guid, EVENT_CONTROL_CODE_ENABLE_PROVIDER,
                     TRACE_LEVEL_INFORMATION, keyword, 0, 0, &params);
  ...
  if (enableErr != ERROR_SUCCESS) {
    ...
    ControlTraceW(sessionHandle, nullptr, &stopProps.props,
                  EVENT_TRACE_CONTROL_STOP);
...
EtwSession::EtwSession(EtwSession &&other) noexcept
    : name_{std::move(other.name_)}, providerGuid_{other.providerGuid_},
      sessionHandle_{other.sessionHandle_},
      consumerHandle_{other.consumerHandle_},
      callback_{std::move(other.callback_)}, stopping_{other.stopping_} {
...
void EtwSession::stop() {
  ...
  if (consumerHandle != kInvalid) {
    CloseTrace(
        static_cast<TRACEHANDLE>(consumerHandle)); // unblocks ProcessTrace
  }
  if (sessionHandle != kInvalid) {
    auto props = makeProperties(name_);
    ControlTraceW(static_cast<TRACEHANDLE>(sessionHandle), nullptr,
                  &props.props, EVENT_TRACE_CONTROL_STOP);
  }
}
} // namespace vigil::platform
