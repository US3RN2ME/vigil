...
namespace vigil::platform::windows {
   ...
   ProcessToken::ProcessToken(Handle token) noexcept
   ...
   std::vector<std::byte> ProcessToken::query(InfoClass infoClass) const {
      ...
      GetTokenInformation(static_cast<HANDLE>(handle_.native()), nativeClass, nullptr, 0, &needed);
      ...
      if (!GetTokenInformation(static_cast<HANDLE>(handle_.native()), nativeClass, buffer.data(), needed, &needed)) {
         return {};
      }
