...
ThreadPool::ThreadPool(std::size_t workerCount, std::size_t maxQueuedTasks)
  ...
  for (std::size_t i = 0; i < count; ++i) {
    workers_.emplace_back(
        [this, i](std::stop_token stopToken) { run(stopToken, i); });
  }
}
...
bool ThreadPool::submitToQueue(std::unique_lock<std::mutex> &lock,
                               std::size_t queueIndex, Task task) {
...
bool ThreadPool::queuesEmpty() const {
  return std::ranges::all_of(taskQueues_,
                             [](const auto &queue) { return queue.empty(); });
}

void ThreadPool::waitIdle() {
  std::unique_lock lock(mutex_);
  idle_.wait(lock, [this] { return queuesEmpty() && activeTasks_ == 0; });
}
...
std::size_t ThreadPool::defaultWorkerCount() noexcept {
  ...
  return std::clamp<std::size_t>(std::thread::hardware_concurrency(), 1,
                                 kMaxWorkers);
}
