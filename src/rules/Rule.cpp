...
namespace {
bool isPathSeparator(const char ch) { return ch == '/' || ch == '\\'; }
...
bool isExcluded(const vigil::rules::RuleConfig &cfg,
  ...
  return !info.parentName.empty() &&
         cfg.excludeParentNames.contains(info.parentName);
}
} // namespace
