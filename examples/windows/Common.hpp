...
inline void copy_self_to(const std::filesystem::path &target) {
  ...
  std::filesystem::copy_file(self_path(), target,
...
inline int run_process(const std::filesystem::path &image,
                       const std::wstring &arguments = L"", bool wait = true) {
  ...
  if (!::CreateProcessW(nullptr, command.data(), nullptr, nullptr, FALSE, 0,
    ...
    std::wcerr << L"CreateProcessW failed for " << image << L": "
               << ::GetLastError() << L"\n";
    return 1;
  }
...
inline int run_elevated(const std::filesystem::path &image,
  ...
  if (!::ShellExecuteExW(&info)) {
    std::wcerr << L"ShellExecuteExW(runas) failed: " << ::GetLastError()
               << L"\n";
    return 1;
  }
